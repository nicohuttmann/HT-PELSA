...
peptide_dd <- data_dd %>%
  ...
  mutate(across(c("1uM_rep1":"02nM_rep4"), ~sum(.x, na.rm = TRUE), .names = "sum_{col}")) %>% distinct(Stripped.Sequence, .keep_all = TRUE) %>%
  ungroup() 
...
if (!file.exists("export_dd_suniti_peptide_stabilized_R1.csv")) {
  ...
  # output
  write.csv (file = paste0("export_dd_suniti_peptide_stabilized_R1",".csv"),stabilized_peptides,row.names = F)
...
# 5.Fig.3G: Heatmap showing the targets of sunitinib in mouse heart
...
pdf(file = "Fig.3G.pdf",width = 8,height = 8,onefile = FALSE)
