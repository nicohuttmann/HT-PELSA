...
## 5.2 Fig.S5A: GO CC analysis of the proteins that are uniquely identified in the crude lysates
...
ggsave(paste0("Fig.S5A", ".pdf"), width = 8,height = 5.5, useDingbats=FALSE)

```


## 5.3 Fig.S5B and S5C: Barplot and pie charts to compare the dasatinib targets identified in the crude and cleared lysates
...
ggsave("FigS5B.pdf",p,width =6.5,height = 6,units = "cm")
...
pie(crude_unique$n,
    labels = "is.trans.membrane", 
    main = "Fig.S5C_crude_unique")

```


## 5.4 Fig.S5D: Local stability for YES1 

```{r}

## Refer to section 4 to get "bayes_test_peptides_crude.csv"

all_peptides <- read.csv("bayes_test_peptides_crude.csv", header = T)
...
YES1 <- all_peptides %>% 
  ...
  mutate(in.domain = case_when(
    end > 91 & start < 152  ~ "SH3",
    end > 158 & start < 255 ~ "SH2",
    end > 270 & start < 530 ~"Protein kinase",
    TRUE ~ "no"
  ))



POI_filter <- YES1 %>% filter(!(abs> 0.3 & Log10P.Value <2))


write.csv(POI_filter, "dasatinib_YES1_peptides.csv")
...
p <- ggplot() +
  ...
  scale_x_continuous(name="sequence",limits = c(0,543),breaks = c(0,100,200,300,400,500,600,700),expand = c(0, 0)) +
  scale_y_continuous(name=expression(Log["2"]*FC),limits =c(-2.5,2),breaks = c(-2,-1,0,1,2),expand = c(0, 0)) +
  theme( 
  ...
  ggtitle(paste0(protein, ", Dasatinib_crude")) +
...
ggsave("Fig.S5D.pdf",width=5,height=4,dpi = 300)

```


## 5.5 Fig.S5E Local stability for BTK for dasatinib
...
BTK  <- all_peptides %>% 
  ...
  mutate(abs= abs(Log2FC)) %>% 
  mutate(in.domain = case_when(
    start > 214 & start < 274  ~ "SH3",
    start > 281 & start < 377 ~ "SH2",
    start > 581 & start < 588 ~"CAV1-binding",
    TRUE ~ "no"
  ))
...
write.csv(POI_filter, "dasatinib_BTK_peptides.csv")
...
geom_rect(data = shadow_rect, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
  ...
  scale_y_continuous(name=expression(Log["2"]*FC),limits =c(-1,2),breaks = c(-1,0,1,2),expand = c(0, 0)) +
  theme( 
    panel.background = element_rect(color = "black",size=0.5),
    axis.title.x = element_text(colour="black",size=15),
    axis.title.y = element_text(colour="black",size=15),
...
ggsave("Fig.S5E.pdf", width = 5, height = 3, dpi = 300)

```


## 5.6 Fig.S5F Local stability for MAP2K1 for dasatinib
...
write.csv(POI_filter, "dasatinib_MAP2K1_peptides.csv")
...
p <- ggplot() +
  geom_segment(data = POI_filter, aes(x = start, xend = end,
                                      y = Log2FC),linewidth = 1.5,color= "black") +  # Correct placement of the size aesthetic
...
ggsave("Fig.S5F.pdf", width = 3, height = 2.8, dpi = 300)
