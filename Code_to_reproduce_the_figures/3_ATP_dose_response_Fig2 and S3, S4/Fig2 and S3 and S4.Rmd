...
peptide_dd <- data_dd %>% ### ???
  ...
  mutate(across(c("1mM_rep1":"control_rep4"), ~sum(.x, na.rm = TRUE), 
                .names = "sum_{col}")) %>% 
...
# 5.1 Get single concentration HT-PELSA 5 mM for Fig.S3B
...
# step-1：data-processing
  peptide_intensity <- peptide_dd_with_position %>%
    select(Protein.Ids, Genes, First.Protein.Description, Stripped.Sequence, peptide.length, from, to, 
           starts_with("sum_0uM"), starts_with("sum_10uM")) %>%
    distinct(Stripped.Sequence, .keep_all = TRUE) %>%
    ungroup()%>%
...
data.analysis <- data.analysis %>%
  mutate(is.uniprot.atp = ifelse(UID %in% uniprot_atp$Entry,"yes","no"))
...
# 5.2 Get single concentration HT-PELSA 5 mM, treat separately, for Fig.S3B

```{r}

# Please re-run the code above to get the result of ATP-5mM treat separately

ATP_5mM_treatSeparately <- read.csv("htPELSA_bayes_test_5mM_ATP_TreatSeparately.csv", header = T)
...
## Fig.2B and Fig.S3C: pEC50 and CV of pEC50 distributions of ATP-stabilized peptides

```{r}

## The final list can be obtained from section 4.2 above. 

final_list <- read.xlsx("final_list.xlsx", sheet = 1)
...
p
ggsave("Fig.S3C.pdf",p,width =8,height = 8,units = "cm")

```


## Fig.2C: Comparing dose-response results of HT-PELSA and LiP-MS (Fig.2C: peptide level)
...
table(pelsa_stabilized_peptides$is.uniprot.atp)

pelsa_stabilized_peptides_treatSep <- read.csv("affinity_treatmentReps.csv", header = T) %>% 
filter(regulation== "stabilized") %>%
  filter(pEC50_CV < 0.25) %>% 
  select("Peptides","Protein.Group",
         "is.known.atp") %>% 
  rename("PeptideID" = 1,"ProteinID"= "Protein.Group","is.uniprot.atp" = "is.known.atp"  )
...
"pelsa_treatSep" = pelsa_stabilized_peptides_treatSep,
.id = "type") %>%
...
## Fig.S3A: Comparing dose-response results of HT-PELSA (both treat together and treat separately) and LiP-MS 
...
lip_ms_dd <- bind_rows(
 ...
 ## Refer to Fig.4.2 for final_list
...
## You can obtain "Ecoli_ATP_affinity_treatSeparately.csv" through section 1-4 above or HT.PELSA package, using the DIA-NN output report_pr_matrix "ATP_treatSeparately_DD_report.pr_matrix.tsv"


pelsa_stabilized_peptides_treatSep <- read.csv("Ecoli_ATP_affinity_treatSeparately.csv", header = T) %>% 
filter(regulation== "stabilized") %>%
  filter(pEC50_CV < 0.25) %>% 
  select("Peptides","Protein.Group",
         "is.known.atp") %>% 
...
dd_compare_protein <- bind_rows(
  "lip_ms_dd" = lip_ms_dd,
"ht-pelsa" = pelsa_stabilized_peptides,
"ht-pelsa_treatSeparately" = pelsa_stabilized_peptides_treatSep,
.id = "type") %>%
 group_by(type,is.uniprot.atp) %>% 
...
## Plotting Fig.S3A protein level

data_plot_pro <- dd_compare_protein %>%
  pivot_wider(names_from = is.uniprot.atp, values_from = n) %>%
  mutate(Ratio = yes / (no + yes))%>%
  pivot_longer(cols = c(yes, no), names_to = "Group", values_to = "n") %>% 
  mutate(type =  factor(type, levels = c("lip_ms_dd","ht-pelsa","ht-pelsa_treatSeparately")))
...
p <- ggplot(data=data_plot_pro)+
  ...
  scale_fill_manual(name="known.ATP.binders",breaks=c("yes","no"),values = c("#a995c4","gray75"))+
  theme_classic()+
  annotate(x=rep(c("lip_ms_dd", "ht-pelsa","ht-pelsa_treatSeparately"),times=2),y=c(30,90,100,100,250,270),label=c("52","174","191", "123","127","138"),geom = "text",size=2)+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size=5,color="black",margin = margin(2,0,-4.5,0)),
        axis.title.y = element_text(size = 6,margin = margin(0,0,0,-3)),
        axis.text.y = element_text(size = 5,colour = "black",margin = margin(0,0.8,0,0)),
...
p
ggsave("Fig.S3A.pdf",p,width =6.5,height = 6,units = "cm")
...
## Fig.S3B: Comparing sing-conc results of HT-PELSA (both treat together and treat separately) and LiP-MS 
...
## PELSA by treatment separately
pelsa_5mM_treatSeparately <- read.csv("htPELSA_bayes_test_5mM_ATP_TreatSeparately.csv") %>%
  distinct(UID,.keep_all = TRUE) %>%
  filter(Log2FC < 0) %>% mutate(
    rank = row_number(),
    atp_count = cumsum(is.uniprot.atp == "yes"),
  ...
  group_by(is.uniprot.atp) %>%
  count() %>%
  mutate(type= "PELSA_5mM_treatSeparately")




combine <- bind_rows(lip_ms_5mM,lip_ms_25mM,
                     pelsa_5mM,
                     pelsa_5mM_treatSeparately )
...
p <- ggplot(data=data_plot)+
  ...
  scale_y_continuous("protein number",breaks = c(0,50,100,150,200,250,300),limits = c(0,310))+
  scale_x_discrete(expand=c(0.1,0.1))+
  scale_fill_manual(name="known.ATP.binders",breaks=c("yes","no"),values = c("#a995c4","gray75"))+
  theme_classic()+
  annotate(x=rep(c("LiP_5mM", "LiP_25mM", "PELSA_5mM","PELSA_5mM_treatSeparately"),times=2),y=c(30,40,90,100,110,125,200,250),label=c("66","84","172","185","96","147","110","117"),geom = "text",size=2) + theme(axis.title.x = element_blank(),
        axis.text.x = element_text(size=5,color="black",margin = margin(2,0,-4.5,0)),
        axis.title.y = element_text(size = 6,margin = margin(0,0,0,-3)),
        axis.text.y = element_text(size = 5,colour = "black",margin = margin(0,0.8,0,0)),
        legend.position = "right",
...
ggsave("FigS3B.pdf",p,width =7,height = 6,units = "cm")

```


## Fig.S3D: pEC50 distribution of all ATP-destabilized peptides
```{r}
...
ggsave("Fig.S3D.pdf",p,width =8,height = 8,units = "cm")

```


## Fig.S3E: Heatmap of all ATP-destabilized proteins
```{r}
...
pdf(file = "Fig.S3E.pdf",width = 8,height = 8,onefile = FALSE)
...
## Fig.S4A and S4B: GO analysis of the high-affinity ATP-destabilized proteins
...
## Fig.S4C: PPI analysis of the high-affinity ATP-destabilized proteins

```{r}


proportion <- read.csv("proportations_of_PPI_of_destabilized.csv", header = T) 
   
 
distance <- read.csv("distance_betweenGP.csv", header = T) %>% 
   dplyr::slice(c(1,2,4,6))

 
 
p <- ggplot(data=proportion)+
  geom_bar(aes(x=category_pair,y=proportion_connected),stat = "identity",width = 0.4, fill = "#4ba473")+
   geom_text(aes(x = category_pair, y = proportion_connected, 
                label = round(proportion_connected, 3)), 
            vjust = -0.5, size = 2.5) +
  scale_y_continuous(limits = c(0,0.088),breaks = seq(0,0.08,by=0.02))+
  xlab("Category Pair")+
  ylab("Proportion of Direct Interactions in BIOGRID")+ 
  theme_classic()+
mytheme+
...
ggsave("Fig.S4C.pdf",p,width =7,height = 6,units = "cm")
